  ...
  if (!opt) {
	...
	(1.0-fl[swlidx[0]]/co[swlidx[0]])*dwl*sp_frac;
      esum_temp=(er[swlidx[0]]<=0.0 || co[swlidx[0]]==0.0) ? 0.0 :
        er[swlidx[0]]/co[swlidx[0]]*dwl;
      esum=esum_temp*esum_temp/sp_frac;
    }
    else {
      sum=(er[swlidx[0]]<=0.0) ? 0.0 : (1.0-fl[swlidx[0]])*dwl*sp_frac;
      esum_temp=(er[swlidx[0]]<=0.0) ? 0.0 : er[swlidx[0]]*dwl;
      esum=esum_temp*esum_temp/sp_frac;
    }
    /* Last pixel */
	...
	esum_temp=(er[ewlidx[0]]<=0.0 || co[ewlidx[0]]==0.0) ? 0.0 :
	  er[ewlidx[0]]/co[ewlidx[0]]*dwl;
	esum+=esum_temp*esum_temp/ep_frac;
      }
      else {
	sum+=(er[ewlidx[0]]<=0.0) ? 0.0 : (1.0-fl[ewlidx[0]])*dwl*ep_frac;
	esum_temp=(er[ewlidx[0]]<=0.0) ? 0.0 : er[ewlidx[0]]*dwl;
	esum+=esum_temp*esum_temp/ep_frac;
      }
    }
    /* Other pixels */

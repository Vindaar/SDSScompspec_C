...
\t%s",spec->file);
  if (hdunum!=14)
    errormsg("SDSS_rspec(): Number of HDUs is %d instead of %d\n\
...
\tfrom FITS file %s.","FIBERID",spec->file);
  ...
  // TODO: RAOBJ and DECOBJ seems to have been renamed to PLUG_RA and PLUG_DEC in DR10.
  // Rather complicated for now.

  // DR5 first, DR10 second
  int status10 = 0;
  fits_read_key(infits,TDOUBLE,"RAOBJ",&(spec->ra),comment,&status);
  fits_read_key(infits,TDOUBLE,"PLUG_RA",&(spec->ra),comment,&status10);

  if(!status && !status10) {
	  fits_report_error(stderr, status);
	  fits_report_error(stderr, status10);
  ...
  // DR5 first, DR10 second
  fits_read_key(infits,TDOUBLE,"DECOBJ",&(spec->dec),comment,&status);
  fits_read_key(infits,TDOUBLE,"PLUG_DEC",&(spec->dec),comment,&status10);
  if(!status && !status10){
	  fits_report_error(stderr, status);
	  fits_report_error(stderr, status10);
...
\tfrom FITS file %s.","MAG",spec->file);
	  if (sscanf(magstr,"%lf %lf %lf %lf %lf",&(spec->mag[0]),&(spec->mag[1]),
		  ...
		  errormsg("SDSS_rspec(): Cannot read fibre mags. from MAG string\n\
\tin header of file\n\t%s",spec->file);
	...
	\tfrom FITS file %s.","PRIMTARG",spec->file);
	  ...
	  if (!SDSS_primtarg(&(spec->tsf)))
	    errormsg("SDSS_rspec(): Error returned from SDSS_primtarg()");
  }
  ...
  if(spec_bool){
	  ...
	  // TODO: Check hdutype
	  fits_movabs_hdu(infits, 3, &hdutype, &status10);
	  int colnum_psfmag = 0;
	  int colnum_primtarg = 0;
	  fits_get_colnum(infits, CASEINSEN, "PSFMAG", &colnum_psfmag, &status10);
	  double psfmag[5];
	  fits_read_col(infits, TDOUBLE, colnum_psfmag, 1, 1, 5, &nulval, &psfmag, &anynul, &status10);
	  for(i = 0; i < 5; i++) printf("psfmag: %f\n", psfmag[i]);

	  // Get Primtarg
	  fits_get_colnum(infits, CASEINSEN, "PRIMTARG", &colnum_primtarg, &status10);
	  fits_read_col(infits, TINT, colnum_primtarg, 1, 1, 1, &nulval, &(spec->tsf.primtarg), &anynul, &status10);
	  if (!SDSS_primtarg(&(spec->tsf)))
	    errormsg("SDSS_rspec(): Error returned from SDSS_primtarg()");
...
\tfrom FITS file %s.","Z",spec->file);
  ...
  if (fits_get_img_param(infits,9,&bitpix,&naxis,naxes,&status))
    errormsg("SDSS_rspec(): Couldn't get image dimensions for\n\
\tfile %s",spec->file);
  if (!naxis) errormsg("SDSS_rspec(): Couldn't find input image extension\n\
\tfor file %s",spec->file);
  else if (naxis!=2) errormsg("SDSS_rspec(): Abnormal number of dimensions\n\

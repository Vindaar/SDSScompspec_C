...
\tof size %d",n);
  ...
  for (i=0; i<n; i++) {
	...
	for (k=0; k<n; k++) {
	  if (ipiv[k]==0) {
	    if ((dum=fabs(a[j][k]))>=big) {
	      big=dum; irow=j; icol=k;
	    }
	  }
    ...
    /*if (a[icol][icol]==0.0) errormsg("gaussj(): Singular matrix (2) formed");*/
    if (a[icol][icol]==0.0) {
      a[icol][icol]=FPMIN;
      ret=GJSNGLR;
      nferrormsg("gaussj(): Singular matrix (2) formed - setting entry\n\
\tto FPMIN to prevent division by zero.");
    }
